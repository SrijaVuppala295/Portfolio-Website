...
const Hero = () => {
  ...
  const textToType = "Building with Node.js, Express.js, HTML, CSS & JS | Exploring Google Colab";
      ...
      <div className="container mx-auto px-4 relative z-10 max-w-7xl">
        <div className="flex flex-col lg:flex-row items-center justify-between gap-12">
          ...
          <div className="lg:w-1/2 text-center lg:text-left mb-12 lg:mb-0">
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold font-playfair mb-3 leading-snug">
              Hi, I'm <span className="block text-neon-green neon-text-animated font-playfair mt-2">Your Name</span>
            </h1>
            <div className="flex flex-col sm:flex-row  items-center sm:justify-center lg:justify-start font-semibold gap-2 sm:gap-5 md:text-lg mb-1 mt-3">
              <span className="bg-neon-green/10 text-neon-green px-3 py-1 rounded-full">Full Stack Web Dev</span>
              <span className="bg-neon-green/10 text-neon-green px-3 py-1 rounded-full">Machine Learning Enthusiast</span>
              <span className="bg-neon-green/10 text-neon-green px-3 py-1 rounded-full">Open Source Contributor</span>
            </div>
            <div className="text-base sm:text-xl md:text-2xl font-mono mb-6 mt-3 overflow-x-auto break-words whitespace-normal">
              <span>{typedText}</span>
              <span className={`inline-block w-2 h-5 ml-1 bg-neon-green align-bottom transition-opacity duration-300 ${showCursor ? 'opacity-100' : 'opacity-0'}`}></span>
            </div>
            <p className="text-md sm:text-base md:text-lg text-muted-foreground mb-8 max-w-xl mx-auto lg:mx-0">
              Always learning and building. Passionate about technology, teamwork, and the open source community.
            </p>
            {/* Tech stack images */}
            <div className="flex flex-wrap justify-center lg:justify-start gap-4 mb-7 mt-2">
              {techStack.map(({ name, src }) => (
                <div key={name} className="flex flex-col items-center group">
                  ...
                  <span className="text-xs text-muted-foreground mt-1 group-hover:text-neon-green transition">{name}</span>
                </div>
              ))}
            </div>
            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
              <a href="#achievements" className="btn btn-primary">View Achievements</a>
        ...
        <div className="w-full flex justify-center z-20 relative mt-14 mb-2">
          <a href="#about" className="flex flex-col items-center text-neon-green">
            ...
            <svg width="22" height="22" fill="none" stroke="currentColor" strokeWidth="2" className="animate-bounce" viewBox="0 0 24 24"><path d="M19 9l-7 7-7-7"/></svg>
          </a>
        </div>
      </div>
    </section>
